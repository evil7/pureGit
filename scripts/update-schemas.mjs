/**
 * GitHub API Schema 资产一键更新脚本（debug 面板数据源，单一入口）
 *
 * 从项目已安装的 octokit 包直接转录两种 schema（**零下载**，与当前 SDK 版本完全同步）：
 ...
 *   含全部 description——不转义不精简直接输出；gzip 204KB / brotli 117KB）
 *
 * **可选升级组件（下载最新 schema 包）**：`pnpm update:schemas --upgrade` 先升级
 * @octokit/openapi + @octokit/graphql-schema 到 npm 最新（根 devDependencies），再转录
 * ——schema 数据源跟随官方更新，产物同步刷新。
 *
 ...
 *
 * 用法：`pnpm update:schemas`（转录已安装 SDK）/ `pnpm update:schemas --upgrade`（先升级再转录）
 */
...
/** 升级两个 schema 数据源包到 npm 最新（根 devDependencies 统一管理） */
async function upgradePackages() {
  const { execSync } = await import("node:child_process");
  console.log("[upgrade] 升级 @octokit/openapi + @octokit/graphql-schema 到最新…");
  execSync("pnpm up @octokit/openapi @octokit/graphql-schema", {
    stdio: "inherit",
    cwd: root,
  });
  console.log("[upgrade] 升级完成，开始转录…");
}
...
if (useUpgrade) await upgradePackages();
// 先清空旧产物目录再转录——schema 更新后 tag/类型集合变化，避免残留已不存在的冗余数据文件
...
if (useUpgrade) console.log("提示：schema 包已升级 + 三层产物已刷新");
else console.log("提示：升级 schema 组件用 `pnpm update:schemas --upgrade`");
