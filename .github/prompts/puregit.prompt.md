...
- **API 策略**：**Octokit SDK 统一封装**（`@octokit/rest` + `@octokit/graphql`，入口 `web/src/lib/octokit.ts`）；**主模式由用户在偏好设置切换**（GraphQL 优先 / REST 优先，默认 GraphQL）；统一经 `web/src/lib/api.ts` 智能封装层，页面组件不感知具体协议；REST core 与 GraphQL **双额度分开计数**、耗尽/熔断自动冗余切换另一模式**不改变设置项**；**匿名强制 REST**（GraphQL 匿名恒 403）。REST 固定端点一律 `typedRequest` + `octokit.rest.*` 类型化方法。
