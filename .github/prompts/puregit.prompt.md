...
### 4.1 改造 GitHub 官方页面（标准 5 步流程）

> 任何「改造 / 复刻某个官方页面」任务**必须**按此流程推进（本项目核心玩法：功能对齐官方、UI 自定义）。详细方法论见 `.github/skills/replica-workflow/SKILL.md`。

1. **对照评估**：对照本项目现有页面，评估改造难度 / 影响 / 收益 / 大致计划流程。
2. **讨论确认**：与用户讨论难点、疑点、待定项，确认核心差异处理方案。
   - 2.1 必要时可**完全重构**难以直接修改到位的文件（破坏性更新，0.0.x 阶段不保兼容）。
3. **实施改造**：按方案改造实施；再按「精简界面、纯化功能」思路提出优化建议。
4. **决策升华**：待用户确认最终决策方案，进行已改造页面的改造升华。
5. **文档同步**：关键项修订到对应公开框架文档（architecture/design/api-compat/vision）；必要时重启 dev 进程；新增组件需发起 rebuild（`pnpm build`）。
   - 5.1 重大改动后**审计所有文档统一对齐**描述与不一致项；对数据格式等底层调整时触发 clean 重置缓存（`node scripts/clean.mjs`）。
