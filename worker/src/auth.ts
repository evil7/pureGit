...
/**
 * 读取会话中记录的 login（仅用于限流标识/日志，**非鉴权判定**）。
 * 注意：OAuth 回调时 worker 请求 /user 网络降级会导致 login 为空字符串——
 * 判定「是否登录」必须用 getSessionToken（有 token 即登录），
 * 不要用本函数（login 空 ≠ 未登录，会导致代理误判 401）。
 * 未登录 / 会话失效 / login 缺失均返回 null。仅读取，不更新 lastSeen。
 */
 ...
 *   - security_events：安全告警读取（Dependabot/Code scanning/Secret scanning alerts 必需；
 *     read/write 两模式均需——alerts 列表/详情/状态更新全走该 scope）
 */
function buildGitHubScope(s: LoginScopes): string {
  if (s.mode === "write") {
    return "repo admin:org user gist admin:public_key delete_repo workflow notifications admin:gpg_key project security_events";
  }
  return "repo read:org read:user user:email read:public_key read:gpg_key read:project security_events";
}
