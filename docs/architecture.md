...
| 职责                 | 说明                                                                                                                                                                             |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OAuth2 令牌管理      | `/$auth/login` 跳转 GitHub 授权；`/$auth/callback` 用 `client_secret` 换取 access token，写入 KV 并下发会话 cookie；`/$auth/pat` **PAT 直接登录**（GitHub 主站受限时绕过授权页，PAT 验证后存 KV + cookie）；`/$auth/session` 按会话恢复 token 返回前端；`/$auth/logout` 注销当前设备；`/$auth/logout/all` 登出全部设备（删当前用户全部 KV 会话，GitHub 端授权保留）；`/$auth/sessions` 会话列表（元数据）；`/$auth/sessions/:id/logout` 本地登出指定设备；`/$auth/revoke` 撤销 OAuth App 授权 |
...
**系统路由优先级**：系统前缀保留段（`/$auth`、`/$wiki`、`/$raw`、`/$healthz`、git 端点 `owner/repo.git/...`）**优先于**用户级通配 `/:owner/:repo`。`$` 符号前缀：GitHub 用户名/仓库名规范**不含 `$`**（仅字母数字+连字符）→ 系统前缀永不被用户路由占用（`$` 明确标识内部高优先级功能性路由）。**`/$debug` 为纯前端路由**（App.tsx lazy 页，worker 不参与）。判断顺序（`worker/src/index.ts`）：`/$healthz`（无条件探活）→ auth（switch）→ 系统代理（`/$wiki`/`/$raw` 含匿名闸）→ git → 前端静态资源（SPA fallback，`/$debug` 由前端路由接管）。**API 调试工具（`/$debug`）schema 数据源**：**直接转录已安装的 octokit 包**——REST `@octokit/openapi`（deref）/ GraphQL `@octokit/graphql-schema`（introspection 原数据）经 `scripts/update-schemas.mjs` → `web/public/debug/`（rest 三层 + gql/schema.json），零下载离线可用；`pnpm update:schemas --upgrade` 先升级两包到 npm 最新再转录（跟随官方更新，单一 schema 源头）。刷新按钮带 token 在线 introspection 兜底（仅内存缓存）；驱动左栏 Schema 树（字段可展开返回类型子字段浏览）+ 编辑器智能补全（cm6-graphql）。
...
```mermaid
flowchart TD
    A[页面组件] --> B[api.ts smart 层<br/>GraphQL 唯一主通道]
    B --> C{GraphQL 可用?<br/>未耗尽 / 未熔断 / 未报错}
    C -->|是| D[GraphQL 请求模板<br/>graphql.ts 模板 + 路径参数变量]
    C -->|否| E[熔断框架：cooldown / 额度耗尽]
    E --> F[withRestFallback 降级链<br/>复用 rest 层现有实现 + ↪ 日志]
    D --> G[返回数据]
    F --> G
```
...
1. GraphQL HTTP 非 2xx / 网络错误（触发 cooldown 60s）
2. 响应 `errors` 字段非空（GraphQL 字段级错误亦然）
3. 请求超时（Octokit request timeout 8s）
4. GraphQL 额度耗尽（`isExhausted`：remaining ≤ 0）
5. 未携带 token → 匿名直接 REST 数据层（非降级，硬约束）

> **1~4 命中 → `withRestFallback` 熔断降级 REST**（复用 rest 层现有实现；日志自动 `↪` 前缀）；
> 仅 5（匿名）是硬约束直接走 REST。GraphQL 主请求日志含 vars + error 详情行（api-log.ts）。
...
### 全部前端功能（携带 token 直连 GitHub GraphQL；匿名走 REST 数据层）
...
sequenceDiagram
    ...
    B->>G: POST /graphql query(仓库信息)（Bearer token，模板+变量）
    G-->>B: 数据（按需字段聚合）
    B->>G: POST /graphql mutation(star 仓库)
    G-->>B: 结果
    Note over B,G: GraphQL 不可达/报错/耗尽时（熔断）
    B->>R: withRestFallback 降级链（复用 rest 层，日志 ↪ 标记）
    R-->>B: 数据
    Note over B,G: 匿名时（GraphQL 恒 403）
    B->>R: GET /repos/owner/repo（匿名 REST 数据层，硬约束）
    R-->>B: 数据
    B->>B: 渲染 / 更新 UI
```
