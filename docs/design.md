...
> 配套 Skill：`.github/skills/ui-layout/SKILL.md`（速查版）与 `.github/skills/shadcn-ui/SKILL.md`（组件添加流程）。
...
| **shadcn 原生** | **自定义阶段**：组件构造直接使用 shadcn 官方组件**默认样式**（尺寸/颜色/间距/圆角），非必要不对原始 shadcn 样式手动调整；确需定制走主题 CSS 变量，禁止散落尺寸覆盖与硬编码色值 |
