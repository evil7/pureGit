...
- **编辑器高度自适应拉满（默认行为，无 fill 开关）**：请求区 Body/Query/Variables 编辑器与响应区返回体——**必须用 `h-full min-h-0` 确定高度链**（外层 scroll 容器高度确定 → 容器 `height:100%` → CodeEditor 外层 `h-full` → 内容区 `grow` → cm-editor `flex:1` 依次解析撑满）；**禁止 `min-h-full`**（只设 min-height 不设 height，flex 高度链 indeterminate → CM6 的 `height:100%` 无法解析 → cm-editor/cm-content 塌陷成内容高：空内容只显一行 + 下方大片空白，内容多时被 overflow 裁剪无法滚动——2026-08-11 浏览器实测逮住）。CodeEditor/CodeView **默认撑满**（外层 flex-col + h-full；内容区 `grow flex-col` + `minHeight` 兜底——父非 flex/内容 1 行时也撑满设定 minHeight，无底部空白；内容多时内部滚动）。**CM6 高度链（2026-08-14 修复）**：CM6 默认 `.cm-scroller{height:100%}`、`.cm-gutters{height:100%}`、`.cm-gutter{min-height:100%}` 与 `.cm-content{min-height:100%}` 都是**百分比解析**——依赖父 computed height 确定；flex 撑满的容器（computed height auto）会让 scroller/content/gutters 塌陷成内容高（gutter 分割线不到底）。修复：theme 里 `.cm-scroller` 加 `flex:1 1 auto; minHeight:0`（相对 cm-editor flex column 撑满）+ `align-items:stretch !important`（交叉轴垂直拉伸 content 到 scroller 全高，绕开百分比链）；custom.css 里 `.cm-gutters{height:auto !important; align-self:stretch !important}`（去掉 height:100%——stretch 只作用于 height:auto 子项）+ `.cm-gutter{min-height:0 !important; align-self:stretch !important}`（gutter 由 gutters 拉伸；sticky 定位不影响交叉轴 stretch）——任何高度链（确定/flex/min-h 兜底）都稳。响应区返回体 pretty 态 CodeEditor 默认撑满、raw 态 `<pre>` 用 `min-h-full`（内容少时占满容器、内容多时自然撑高外层滚动）——两个展示框均与内容区等高，不留底部空白
