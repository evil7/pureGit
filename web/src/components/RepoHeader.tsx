...
const TABS: {
  ...
  { to: "", labelKey: "repoHeader.tabs.code", icon: Code },
  { to: "/issues", labelKey: "repoHeader.tabs.issues", icon: CircleDot, feature: "issues" },
  { to: "/pulls", labelKey: "repoHeader.tabs.pulls", icon: GitPullRequest },
  {
    to: "/discussions",
    labelKey: "repoHeader.tabs.discussions",
    icon: MessagesSquare,
    feature: "discussions",
  },
  { to: "/actions", labelKey: "repoHeader.tabs.actions", icon: Play },
  { to: "/projects", labelKey: "repoHeader.tabs.projects", icon: Table2, feature: "projects" },
  { to: "/wiki", labelKey: "repoHeader.tabs.wiki", icon: BookOpen, feature: "wiki" },
  { to: "/security", labelKey: "repoHeader.tabs.security", icon: Shield },
  { to: "/pulse", labelKey: "repoHeader.tabs.insights", icon: ChartLine },
  { to: "/settings", labelKey: "repoHeader.tabs.settings", icon: Settings },
];
...
export default function RepoHeader({
  ...
  // Actions metrics（usage/performance）URL 虽以 /actions 开头，但官方归属 Insights 分区
  // （Insights 左栏「Actions usage metrics / performance metrics」），故单独识别，
  // 避免「自动化」「洞察」两个 tab 同时高亮。
  const isActionsMetricsPage = /^actions\/metrics\//.test(rest);
  // Insights tab 高亮：官方洞察系列子页（pulse / graphs/* / community / network / forks /
  // actions metrics usage·performance）共用顶部 Insights tab，点击任一子项不改变 Insights 高亮。
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            {data && (
              <div className="ml-auto flex shrink-0 items-center gap-2 pl-4">
                {/* 模板仓库：Use this template → 新建仓库页（携带 template_owner/template_name） */}
                {data.is_template && (
                  <Button asChild variant="outline" size="sm">
                    <Link to={`/new?template_owner=${owner}&template_name=${repo}`}>
                      {t("repoHeader.useTemplate")}
                    </Link>
                  </Button>
                )}
          ...
          {TABS.map((tab) => {
            ...
            const isActive =
              tab.to === "/pulse"
                ? isInsightsActive
                : tab.to === "/actions"
                  ? current === "actions" && !isActionsMetricsPage
                  : tab.to
                    ? current === tab.to.slice(1)
                    : isCodeActive;
