...
export default function RepoLayout() {
  ...
  // Insights 图表子页（graphs/*，contributors/traffic/commits/code-frequency）与 Community 标准页：
  // 官方全宽单列，无 About 右栏（与 pulse 一致；这些页自带 InsightsShell 左栏 InsightsNav）
  const isGraphsPage = /\/graphs(\/|$)/.test(pathname);
  const isCommunityPage = /\/community(\/|$)/.test(pathname);
  ...
  // Forks 列表页（复数；官方同 pulse 单列，InsightsShell 自带左栏，无 About 右栏）
  const isForksPage = /\/forks(\/|$)/.test(pathname);
  ...
  useEffect(() => {
    // About 侧栏 Tags 计数（REST-only；失败/限流静默保持 0 → 无 tags 时不展示 Tags 项）
    if (hideAbout) return;
    let cancelled = false;
    fetchTagsSmart(owner!, repo!, token)
      .then((tags) => !cancelled && setTagsCount(tags.length))
      .catch(() => {});
    return () => {
      cancelled = true;
    };
  ...
  /** 取消归档（ADMIN 权限；成功后局部同步 archived/archived_at） */
  const handleUnarchive = async () => {
    if (!token || unarchiving) return;
    setUnarchiving(true);
    try {
      await updateRepositorySmart(owner!, repo!, token, { archived: false });
      toastSuccess(t("repoArchived.unarchived"));
      setData((prev) => (prev ? { ...prev, archived: false, archived_at: null } : prev));
    } catch (e) {
      toastError(apiErrorMessage(e, t("repoArchived.unarchiveFailed")));
    } finally {
  ...
  return (
    <div className="min-h-svh">
      ...
      >
        ...
        />
        {/* 归档告警横幅（tabs 下方全宽单行；ADMIN 可快捷取消归档） */}
        <ArchivedBanner
          archivedAt={data.archived_at}
          canAdmin={data?.viewer_permission === "ADMIN"}
