...
/** 设置 web 提交签名（独立 mutation；UpdateRepositoryInput 无此字段，须走本 mutation；REST web_commit_signoff_required 可降级） */
export const UPDATE_REPOSITORY_WEB_COMMIT_SIGNOFF_SETTING_MUTATION = /* GraphQL */ `
  mutation UpdateRepositoryWebCommitSignoffSetting(
    $repositoryId: ID!
    $webCommitSignoffRequired: Boolean!
  ) {
    updateRepositoryWebCommitSignoffSetting(
      input: { repositoryId: $repositoryId, webCommitSignoffRequired: $webCommitSignoffRequired }
    ) {
      repository {
        id
        nameWithOwner
