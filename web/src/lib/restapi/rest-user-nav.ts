...
/** Gist 提交历史（GET /gists/{id}/commits 单项） */
...
/** Gist 提交历史（GET /gists/{id}/commits） */
export async function fetchGistRevisions(
  id: string,
  token?: string | null,
  perPage = 30,
  page = 1,
): Promise<GistRevision[]> {
  const data = await typedRequest<GistRevision[]>(token, (octokit) =>
    octokit.rest.gists.listCommits({ gist_id: id, per_page: perPage, page }),
  );
  return data ?? [];
}

/** 获取 gist 某个历史版本（GET /gists/{id}/{sha}，返回该版本文件内容） */
export async function getGistRevision(
  id: string,
  sha: string,
  token?: string | null,
): Promise<Gist> {
  return typedRequest<Gist>(token, (octokit) =>
    octokit.rest.gists.getRevision({ gist_id: id, sha }),
  );
}
...
/** Gist 评论（GET /gists/{id}/comments 单项） */
...
/** 列出 gist 评论（GET /gists/{id}/comments） */
export async function fetchGistComments(
  id: string,
  token?: string | null,
  perPage = 100,
): Promise<GistComment[]> {
  const data = await typedRequest<GistComment[]>(token, (octokit) =>
    octokit.rest.gists.listComments({ gist_id: id, per_page: perPage }),
  );
  return data ?? [];
}

/** 发表 gist 评论（POST /gists/{id}/comments；需 gist scope） */
export async function createGistComment(
  id: string,
  body: string,
  token: string,
): Promise<GistComment> {
  return typedRequest<GistComment>(token, (octokit) =>
    octokit.rest.gists.createComment({ gist_id: id, body }),
  );
}

/** 更新 gist 评论（PATCH /gists/{id}/comments/{comment_id}；需 gist scope） */
export async function updateGistComment(
  ...
  return typedRequest<GistComment>(token, (octokit) =>
    octokit.rest.gists.updateComment({ gist_id: gistId, comment_id: commentId, body }),
  );
}

/** 删除 gist 评论（DELETE /gists/{id}/comments/{comment_id}；需 gist scope） */
export async function deleteGistComment(
  ...
  await typedRequest<void>(token, (octokit) =>
    octokit.rest.gists.deleteComment({ gist_id: gistId, comment_id: commentId }),
  );
}

/** fork gist（POST /gists/{id}/forks；需 gist scope，返回 fork 后的 gist） */
export async function forkGist(id: string, token: string): Promise<Gist> {
  return typedRequest<Gist>(token, (octokit) => octokit.rest.gists.fork({ gist_id: id }));
}

/** 删除 gist（DELETE /gists/{id}；需 gist scope） */
export async function deleteGist(id: string, token: string): Promise<void> {
  await typedRequest<void>(token, (octokit) => octokit.rest.gists.delete({ gist_id: id }));
}

/** 认证用户 Starred 的 gists（GET /gists/starred；需登录） */
export async function fetchStarredGists(token: string, perPage = 30, page = 1): Promise<Gist[]> {
  return typedRequest<Gist[]>(token, (octokit) =>
    octokit.rest.gists.listStarred({ per_page: perPage, page }),
  );
}

/** 公开 gists 浏览（GET /gists/public；匿名可，按最近更新排序） */
export async function fetchPublicGists(
  ...
  return typedRequest<Gist[]>(token, (octokit) =>
    octokit.rest.gists.listPublic({ per_page: perPage, page }),
  );
}

/** 某用户公开 gists（GET /users/{username}/gists） */
export async function fetchUserGists(
  ...
  return typedRequest<Gist[]>(token, (octokit) =>
    octokit.rest.gists.listForUser({ username: login, per_page: perPage, page }),
  );
}
...
/** 按仓库标记通知已读：PUT /repos/{owner}/{repo}/notifications（需 notifications scope） */
export async function markRepoNotificationsAsRead(
  owner: string,
  repo: string,
  token: string,
): Promise<void> {
  await typedRequest<void>(token, (octokit) =>
    octokit.rest.activity.markRepoNotificationsAsRead({ owner, repo }),
  );
}
...
/** 单条线程标记 done：DELETE /notifications/threads/{id}（需 notifications scope；从列表移除） */
export async function markThreadAsDone(threadId: string, token: string): Promise<void> {
  await typedRequest<void>(token, (octokit) =>
    octokit.rest.activity.markThreadAsDone({ thread_id: Number(threadId) }),
  );
}

/** 退订线程：DELETE /notifications/threads/{id}/subscription（需 notifications scope） */
export async function deleteThreadSubscription(threadId: string, token: string): Promise<void> {
  await typedRequest<void>(token, (octokit) =>
    octokit.rest.activity.deleteThreadSubscription({ thread_id: Number(threadId) }),
  );
}
...
/**
 * 用户公开活动流：GET /users/{login}/events/public（该用户**自身**的公开动态）
 * 匿名可访问（无需 token）；GraphQL 无 events 查询 → 仅 REST。
 * 结构与 received_events 相同（复用 ReceivedEvent），供用户主页 Activity tab 渲染。
 * @param page 分页（首页 1）
 */
...
/** 代码搜索结果项（GET /search/code；preview） */
...
/** commit 搜索结果项（GET /search/commits；preview） */
...
/** label 搜索结果项（GET /search/labels；需 repository_id 参数） */
...
/** 搜索代码（GET /search/code；GraphQL 无 CODE SearchType → REST-only） */
export async function searchCode(
  ...
  return typedRequest<SearchResponse<CodeSearchItem>>(token, (octokit) =>
    octokit.rest.search.code({ q, per_page: perPage, page }),
  );
}

/** 搜索 commit（GET /search/commits；GraphQL 无 COMMIT SearchType → REST-only） */
export async function searchCommits(
  ...
  return typedRequest<SearchResponse<CommitSearchItem>>(token, (octokit) =>
    octokit.rest.search.commits({ q, per_page: perPage, page }),
  );
}

/** 搜索 label（GET /search/labels；需 repository_id；GraphQL 无 LABEL SearchType → REST-only） */
export async function searchLabels(
  ...
  token?: string | null,
  page = 1,
): Promise<SearchResponse<LabelSearchItem>> {
  return typedRequest<SearchResponse<LabelSearchItem>>(token, (octokit) =>
    octokit.rest.search.labels({ q, repository_id: repositoryId, per_page: perPage, page }),
  );
}
