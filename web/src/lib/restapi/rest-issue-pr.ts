...
/** 安全分析各功能开关状态（REST security_and_analysis；status: enabled/disabled，未配置时可能为 null） */
export interface SecurityAnalysis {
  advanced_security?: { status?: string };
  code_security?: { status?: string };
  secret_scanning?: { status?: string };
  secret_scanning_push_protection?: { status?: string };
  secret_scanning_ai_detection?: { status?: string };
...
export interface Repository {
  ...
  /** Sponsorships 开关（GraphQL-only hasSponsorshipsEnabled；REST 无对应字段） */
  ...
  is_template?: boolean;
  /** Pull Requests 区：允许更新落后分支（REST allow_update_branch） */
  allow_update_branch?: boolean;
  /** 允许 fork（REST allow_forking） */
  allow_forking?: boolean;
  /** 要求 web 提交签名（REST web_commit_signoff_required） */
  web_commit_signoff_required?: boolean;
  /** merge commit 标题/消息格式（REST merge_commit_title/message，PR_TITLE 等枚举） */
  merge_commit_title?: string;
  merge_commit_message?: string;
  /** squash commit 标题/消息格式（REST squash_merge_commit_title/message） */
  squash_merge_commit_title?: string;
  squash_merge_commit_message?: string;
  /** 安全分析各功能开关状态（REST security_and_analysis） */
...
export interface Issue {
  ...
  /** 对话是否锁定（issues/lock；REST 详情原生字段 + GraphQL locked） */
  locked?: boolean;
  /** 是否置顶（GraphQL isPinned；REST 详情无此字段，降级时 undefined） */
  isPinned?: boolean;
  /** GraphQL node id（issue 主体 reaction 写入用；REST 通道无此字段） */
  nodeId?: string;
  /** 表情反应组（GraphQL reactionGroups 归一；REST 详情无此字段） */
  reactions?: ReactionGroup[];
  /** 当前用户是否已对该主体做出反应（GraphQL viewerHasReacted） */
...
/** 表情反应组（ReactionGroup：content 为 ReactionContent 枚举，count 为 reactors 总数；REST/GraphQL 归一） */
export interface ReactionGroup {
  content: string;
  count: number;
  /** 当前用户是否对该表情做出反应（GraphQL ReactionGroup.viewerHasReacted；精确高亮 pill） */
  viewerHasReacted?: boolean;
}
...
export interface IssueComment {
  ...
  user: GitHubUser;
  html_url: string;
  /** GraphQL node id（评论 reaction 写入用；REST 通道无 node_id 时 undefined） */
  nodeId?: string;
  /** 表情反应组（GraphQL reactionGroups 归一；REST 通道无此字段） */
  reactions?: ReactionGroup[];
  /** 当前用户是否已对该评论做出反应（GraphQL viewerHasReacted） */
  viewerHasReacted?: boolean;
}
...
export interface RepoLabel {
  id: number;
  /** GraphQL node id（编辑/删除 label mutation 用；REST 通道无此字段） */
  nodeId?: string;
  name: string;
  color: string;
  description?: string | null;
...
/** 仓库里程碑（GET /repos/{o}/{r}/milestones；侧栏 Set milestone 编辑弹窗数据源 + 管理页） */
...
/** 仓库里程碑（按状态；管理页 Open/Closed tab 用） */
export async function fetchRepoMilestonesByState(
  owner: string,
  repo: string,
  state: "open" | "closed",
  token?: string | null,
  ...
  return typedRequest<RepoMilestone[]>(token, (octokit) =>
    octokit.rest.issues.listMilestones({ owner, repo, state, per_page: 100 }),
  );
}

/** label 创建/更新可传字段（官方；name/color 创建必填） */
...
/** 新建 label（POST /repos/{o}/{r}/labels；需 write 权限） */
export async function createLabel(
  ...
  return typedRequest<RepoLabel>(token, (octokit) =>
    octokit.rest.issues.createLabel({ owner, repo, ...input }),
  );
}

/** 更新 label（PATCH /repos/{o}/{r}/labels/{name}；需 write 权限） */
...
/** 删除 label（DELETE /repos/{o}/{r}/labels/{name}；需 write 权限） */
export async function deleteLabel(
  ...
  await typedRequest(token, (octokit) => octokit.rest.issues.deleteLabel({ owner, repo, name }));
}

/** milestone 创建/更新可传字段（官方；title 创建必填） */
...
/** 新建 milestone（POST /repos/{o}/{r}/milestones；需 write 权限） */
export async function createMilestone(
  ...
  return typedRequest<RepoMilestone>(token, (octokit) =>
    octokit.rest.issues.createMilestone({
      ...
      title: input.title,
      description: input.description,
      due_on: input.due_on ?? undefined,
    }),
  );
}

/** 更新 milestone（PATCH /repos/{o}/{r}/milestones/{number}；需 write 权限） */
export async function updateMilestone(
  ...
  return typedRequest<RepoMilestone>(token, (octokit) =>
    octokit.rest.issues.updateMilestone({
      ...
      state: input.state,
      description: input.description,
      // due_on 传 null 表示清除截止日期（octokit 类型未声明 null，显式断言保留语义）
      due_on: input.due_on as string | undefined,
    }),
  );
}

/** 删除 milestone（DELETE /repos/{o}/{r}/milestones/{number}；需 write 权限） */
export async function deleteMilestone(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.issues.deleteMilestone({ owner, repo, milestone_number: number }),
  );
}
...
/** 锁定对话（issue 侧栏 Lock conversation；issues/lock 对 issue/PR 通用） */
...
/** 解锁对话（issue 侧栏；issues/unlock 对 issue/PR 通用） */
...
/** 编辑 issue/PR 评论（PATCH /repos/{o}/{r}/issues/comments/{id}） */
...
/** 删除 issue/PR 评论（DELETE /repos/{o}/{r}/issues/comments/{id}；不可恢复） */
export async function deleteIssueComment(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.issues.deleteComment({ owner, repo, comment_id: commentId }),
  );
}
...
/** 编辑 PR 行内评审评论（PATCH /repos/{o}/{r}/pulls/comments/{id}） */
export async function updateReviewComment(
  ...
  return typedRequest<ReviewComment>(token, (octokit) =>
    octokit.rest.pulls.updateReviewComment({ owner, repo, comment_id: commentId, body }),
  );
}

/** 删除 PR 行内评审评论（DELETE /repos/{o}/{r}/pulls/comments/{id}；不可恢复） */
export async function deleteReviewComment(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.pulls.deleteReviewComment({ owner, repo, comment_id: commentId }),
  );
}

/** 回复 PR 行内评审评论（POST /repos/{o}/{r}/pulls/{n}/comments/{id}/replies） */
...
/** 仓库协作者（@ 补全 + 协作者设置页数据源；GET /repos/{o}/{r}/collaborators，仅登录可见） */
export interface Collaborator {
  login: string;
  avatar_url: string;
  /** 权限位（listCollaborators 返回 permissions.admin/push/pull） */
  permissions?: { admin?: boolean; push?: boolean; pull?: boolean };
  /** 角色名（admin/maintain/write/triage/read） */
  role_name?: string;
}
...
/** 添加协作者（PUT /repos/{o}/{r}/collaborators/{username}；permission: pull/push/maintain/triage/admin） */
export async function addCollaborator(
  ...
  await typedRequest<void>(token, (octokit) =>
    octokit.rest.repos.addCollaborator({ owner, repo, username, permission }),
  );
}

/** 移除协作者（DELETE /repos/{o}/{r}/collaborators/{username}） */
export async function removeCollaborator(
  ...
  await typedRequest<void>(token, (octokit) =>
    octokit.rest.repos.removeCollaborator({ owner, repo, username }),
  );
}

/** 查询协作者权限级别（GET /repos/{o}/{r}/collaborators/{username}/permission） */
export async function getCollaboratorPermissionLevel(
  owner: string,
  repo: string,
  username: string,
  token: string,
): Promise<{ permission: string; user: GitHubUser }> {
  return typedRequest<{ permission: string; user: GitHubUser }>(token, (octokit) =>
...
export interface PullReview {
  id: number;
  /** GraphQL node id（驳回/删除评审 mutation 用；REST 通道无此字段） */
  nodeId?: string;
...
/** 驳回评审（PUT /pulls/{n}/reviews/{id}/dismissals；需 write 权限 + 驳回理由） */
...
/** 更新 PR 评审（PATCH /pulls/{n}/reviews/{id}；更新 pending 草稿的 body） */
...
/** 删除 pending 评审草稿（DELETE /pulls/{n}/reviews/{id}；不可恢复） */
...
/** 更新 PR 分支（PUT /pulls/{n}/update-branch；merge base into head，需 write 权限） */
export async function updatePullBranch(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.pulls.updateBranch({ owner, repo, pull_number: number }),
  );
}
