...
import type { Repository, PullFile, SecurityAnalysis, IssueComment } from "./rest-issue-pr";
...
/** 从模板仓库创建（POST /repos/{template_owner}/{template_repo}/generate；GraphQL 无适配 → REST-only） */
export async function createRepositoryFromTemplate(
  ...
  return typedRequest<{ full_name: string }>(token, (octokit) =>
...
export interface UpdateRepoFields {
  ...
  has_wiki?: boolean;
  has_projects?: boolean;
  /** Sponsorships 开关（GraphQL-only；REST update 无对应字段，由 smart 层走 GraphQL） */
  ...
  /** 模板仓库 */
  is_template?: boolean;
  /** Pull Requests 区：允许更新落后分支 */
  allow_update_branch?: boolean;
  /** 允许 fork */
  allow_forking?: boolean;
  /** 要求 web 提交签名 */
  web_commit_signoff_required?: boolean;
  /** merge/squash commit 标题与消息格式（GitHub 官方枚举字面量） */
  merge_commit_title?: "PR_TITLE" | "MERGE_MESSAGE";
  merge_commit_message?: "PR_BODY" | "PR_TITLE" | "BLANK";
  squash_merge_commit_title?: "PR_TITLE" | "COMMIT_OR_PR_TITLE";
  squash_merge_commit_message?: "PR_BODY" | "COMMIT_MESSAGES" | "BLANK";
  /** 安全分析各功能开关（advanced_security/secret_scanning 等） */
...
/** 检查 immutable releases 是否启用（GET /repos/{o}/{r}/immutable-releases；404 视为未启用） */
export async function checkImmutableReleases(
  owner: string,
  repo: string,
  token: string,
): Promise<boolean> {
  try {
    await typedRequest(token, (octokit) =>
      octokit.rest.repos.checkImmutableReleases({ owner, repo }),
    );
    return true;
  } catch {
    return false;
...
/** 启用 immutable releases（POST /repos/{o}/{r}/immutable-releases；需 admin） */
export async function enableImmutableReleases(
  owner: string,
  repo: string,
  token: string,
): Promise<void> {
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.enableImmutableReleases({ owner, repo }),
  );
}

/** 禁用 immutable releases（DELETE /repos/{o}/{r}/immutable-releases；需 admin） */
export async function disableImmutableReleases(
  owner: string,
  repo: string,
  token: string,
): Promise<void> {
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.disableImmutableReleases({ owner, repo }),
  );
}

/** 重命名分支（POST /repos/{o}/{r}/branches/{branch}/rename；需 admin） */
export async function renameBranch(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.renameBranch({ owner, repo, branch, new_name: newName }),
  );
}

/** 删除分支保护规则（DELETE /repos/{o}/{r}/branches/{branch}/protection；需 admin） */
export async function deleteBranchProtection(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.deleteBranchProtection({ owner, repo, branch }),
  );
}
 ...
 * - maxBytes 可选：读 body 前按 Content-Length 拦截，超限抛 413（blob 页内联 1MB 门控）。
 */
...
export interface Release {
  ...
  assets: {
    ...
    /** 资产标签（REST updateReleaseAsset 可改；GraphQL 无此字段） */
    label?: string | null;
    /** SHA256 摘要（REST 返回 `sha256:HEX`；GraphQL ReleaseAsset 无此字段 → 需 REST 补全） */
    digest?: string | null;
  }[];
  /** GraphQL node id（编辑/删除 release mutation 用；REST 通道无此字段） */
  nodeId?: string;
  /** 上传资产模板 URL（REST 原生 upload_url；仅 REST 通道提供） */
  upload_url?: string;
}
...
/** 创建/更新 release 可传字段（对齐官方；tag_name 创建时必填） */
...
/** 创建 release（REST POST /repos/{o}/{r}/releases；需 write 权限） */
export async function createRelease(
  ...
  return typedRequest<Release>(token, (octokit) =>
    octokit.rest.repos.createRelease({ owner, repo, ...input }),
  );
}

/** 更新 release（REST PATCH /repos/{o}/{r}/releases/{id}；需 write 权限） */
export async function updateRelease(
  ...
  return typedRequest<Release>(token, (octokit) =>
    octokit.rest.repos.updateRelease({ owner, repo, release_id: releaseId, ...input }),
  );
}

/** 删除 release（REST DELETE /repos/{o}/{r}/releases/{id}；危险操作，不可恢复） */
export async function deleteRelease(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.deleteRelease({ owner, repo, release_id: releaseId }),
  );
}

/** 上传 release 资产（REST POST upload_url 模板，multipart；GraphQL 无适配 → REST-only） */
...
/** 删除 release 资产（REST DELETE /repos/{o}/{r}/releases/assets/{id}） */
...
/** 更新 release 资产（REST PATCH /repos/{o}/{r}/releases/assets/{id}；name/label 可选） */
export async function updateReleaseAsset(
  owner: string,
  repo: string,
  assetId: number,
  patch: { name?: string; label?: string },
  token: string,
): Promise<Release["assets"][number]> {
  return typedRequest<Release["assets"][number]>(token, (octokit) =>
    octokit.rest.repos.updateReleaseAsset({ owner, repo, asset_id: assetId, ...patch }),
  );
}

/** 自动生成 release notes（REST POST generate-notes；GraphQL 无适配 → REST-only） */
export async function generateReleaseNotes(
  ...
  const res = await typedRequest<{ body?: string }>(token, (octokit) =>
...
/** commit 评论列表（GET /repos/{o}/{r}/commits/{sha}/comments） */
export async function listCommitComments(
  ...
  return typedRequest<IssueComment[]>(token, (octokit) =>
    octokit.rest.repos.listCommentsForCommit({ owner, repo, commit_sha: sha, per_page: 100 }),
  );
}

/** 发表 commit 评论（POST /repos/{o}/{r}/commits/{sha}/comments） */
export async function createCommitComment(
  ...
  return typedRequest<IssueComment>(token, (octokit) =>
    octokit.rest.repos.createCommitComment({ owner, repo, commit_sha: sha, body }),
  );
}

/** 编辑 commit 评论（PATCH /repos/{o}/{r}/comments/{id}） */
export async function updateCommitComment(
  ...
  return typedRequest<IssueComment>(token, (octokit) =>
    octokit.rest.repos.updateCommitComment({ owner, repo, comment_id: commentId, body }),
  );
}

/** 删除 commit 评论（DELETE /repos/{o}/{r}/comments/{id}） */
export async function deleteCommitComment(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.deleteCommitComment({ owner, repo, comment_id: commentId }),
  );
}

/** commit 关联的 PR（GET /repos/{o}/{r}/commits/{sha}/pulls；preview） */
export async function listCommitAssociatedPRs(
  owner: string,
  repo: string,
  sha: string,
  token?: string | null,
  ...
  try {
    const prs = await typedRequest<{ number: number; title: string; html_url: string }[]>(
      token,
      (octokit) =>
...
/** commit 综合状态（GET /repos/{o}/{r}/commits/{sha}/status） */
...
// ===== Interactions（临时交互限制，官方 /settings/interaction_limits）=====

/** 交互限制级别（官方枚举：existing_users / contributors_only / collaborators_only） */
export type InteractionLimitValue = "existing_users" | "contributors_only" | "collaborators_only";

/** 交互限制时长（官方枚举；octokit 类型化，非 ISO 时间戳） */
export type InteractionExpiry = "one_day" | "three_days" | "one_week" | "one_month" | "six_months";

/** 仓库交互限制（GET /repos/{o}/{r}/interaction-limits；无限制时 204 无 body） */
export interface InteractionLimit {
  limit: InteractionLimitValue;
  origin: "repository" | "organization" | "user";
  expires_at: string | null;
}

/** 查询当前交互限制（无限制 → null；REST-only，GraphQL 无仓库级 mutation） */
export async function fetchInteractionLimits(
  owner: string,
  repo: string,
  token: string,
): Promise<InteractionLimit | null> {
  const data = await typedRequest<InteractionLimit | undefined>(token, (octokit) =>
    octokit.rest.interactions.getRestrictionsForRepo({ owner, repo }),
  );
  return data ?? null;
}

/** 设置交互限制（PUT /repos/{o}/{r}/interaction-limits；expiry 可选时长枚举） */
export async function setInteractionLimits(
  owner: string,
  repo: string,
  limit: InteractionLimitValue,
  expiry: InteractionExpiry | undefined,
  token: string,
): Promise<InteractionLimit> {
  return typedRequest<InteractionLimit>(token, (octokit) =>
...
/** 移除交互限制（DELETE /repos/{o}/{r}/interaction-limits） */
export async function removeInteractionLimits(
  owner: string,
  repo: string,
  token: string,
): Promise<void> {
  await typedRequest(token, (octokit) =>
    octokit.rest.interactions.removeRestrictionsForRepo({ owner, repo }),
  );
}

// ===== Stargazers / Watchers 列表（官方 /stargazers 与 /watchers）=====

/** 仓库 stargazers 列表（REST GET /repos/{o}/{r}/stargazers；按 star 时间倒序 + page 分页） */
export async function fetchStargazers(
  ...
  return typedRequest<GitHubUser[]>(token, (octokit) =>
    octokit.rest.activity.listStargazersForRepo({ owner, repo, per_page: perPage, page }),
  );
}

/** 仓库 watchers 列表（REST GET /repos/{o}/{r}/subscribers；page 分页） */
export async function fetchWatchers(
  ...
  return typedRequest<GitHubUser[]>(token, (octokit) =>
    octokit.rest.activity.listWatchersForRepo({ owner, repo, per_page: perPage, page }),
  );
}

/** fork 仓库简版（fork 列表页展示字段，REST/GraphQL 统一收窄） */
...
/** 仓库 forks 列表（REST GET /repos/{o}/{r}/forks；sort=newest + page 分页） */
export async function fetchForks(
  ...
  const data = await typedRequest<RepoFork[]>(token, (octokit) =>
    octokit.rest.repos.listForks({ owner, repo, per_page: perPage, page, sort: "newest" }),
  );
  return data ?? [];
}

// ===== Deploy keys（官方 /settings/keys；写操作 GraphQL 无 mutation → REST-only）=====

/** 仓库 deploy key（REST /repos/{o}/{r}/keys 返回结构） */
...
/** 列出仓库 deploy keys（REST GET /repos/{o}/{r}/keys） */
export async function fetchDeployKeys(
  owner: string,
  repo: string,
  token?: string | null,
): Promise<RepoDeployKey[]> {
  return typedRequest<RepoDeployKey[]>(token, (octokit) =>
    octokit.rest.repos.listDeployKeys({ owner, repo, per_page: 100 }),
  );
}

/** 添加 deploy key（REST POST /repos/{o}/{r}/keys；readOnly 控制是否授予写权限） */
export async function addDeployKey(
  ...
  return typedRequest<RepoDeployKey>(token, (octokit) =>
    octokit.rest.repos.createDeployKey({ owner, repo, title, key, read_only: readOnly }),
  );
}

/** 删除 deploy key（REST DELETE /repos/{o}/{r}/keys/{key_id}；204 无响应体） */
export async function deleteDeployKey(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.deleteDeployKey({ owner, repo, key_id: keyId }),
  );
}

// ===== Environments（官方 /settings/environments；GraphQL 有 mutation → 主通道 + REST 降级）=====

/** 仓库 environment（REST /repos/{o}/{r}/environments 返回结构 + GraphQL 映射补 nodeId） */
export interface RepoEnvironment {
  /** 数字 id（REST databaseId；GraphQL 映射 databaseId） */
  id: number;
  /** GraphQL node id（deleteEnvironment mutation 用；REST 路径无） */
  nodeId?: string;
  name: string;
  /** protection rules 数量 */
...
/** 列出仓库 environments（REST GET /repos/{o}/{r}/environments） */
export async function fetchEnvironments(
  ...
  >(token, (octokit) => octokit.rest.repos.getAllEnvironments({ owner, repo, per_page: 100 }));
  return list.map((e) => ({
    id: e.id,
    nodeId: e.node_id,
    name: e.name,
    protectionRules: e.protection_rules?.length ?? 0,
    created_at: e.created_at,
    updated_at: e.updated_at,
  }));
}

/** 新建 environment（REST PUT /repos/{o}/{r}/environments/{name}；同名即更新） */
export async function createEnvironment(
  ...
  const e = await typedRequest<{
    ...
    octokit.rest.repos.createOrUpdateEnvironment({ owner, repo, environment_name: name }),
  );
  return {
    id: e.id,
    nodeId: e.node_id,
...
/** 删除 environment（REST DELETE /repos/{o}/{r}/environments/{name}；204 无响应体） */
export async function deleteEnvironment(
  ...
  await typedRequest(token, (octokit) =>
    octokit.rest.repos.deleteAnEnvironment({ owner, repo, environment_name: name }),
  );
}
