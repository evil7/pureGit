...
import { notifyModeFallback } from "../ui/toast";
import { logMainRequest, inFallback } from "./api-log";
...
export function setApiMode(mode: ApiMode): void {
  ...
  void import("../auth/prefs-sync").then((m) => m.requestPrefsPush());
}
...
function trackedFetch(
  ...
  // 请求发起时同步捕获降级状态：REST 日志在 res.clone().text() 异步微任务中才输出，
  // 彼时 withRestFallback 可能已 end() 弹栈，动态查 inFallback() 会丢失 ↪ 图标。
  const isFallback = inFallback();
