...
// ===== code / commit / label 搜索：GraphQL 无对应 SearchType → REST-only（红线例外）=====

/** 搜索代码（GET /search/code） */
export async function searchCodeSmart(
  q: string,
  token?: string | null,
  page = 1,
): Promise<SearchResponse<CodeSearchItem>> {
  return searchCode(q, 20, token, page);
}

/** 搜索 commit（GET /search/commits） */
export async function searchCommitsSmart(
  q: string,
  token?: string | null,
  page = 1,
): Promise<SearchResponse<CommitSearchItem>> {
  return searchCommits(q, 20, token, page);
}

/** 搜索 label（GET /search/labels；需 repository_id） */
export async function searchLabelsSmart(
  q: string,
  repositoryId: number,
  token?: string | null,
  page = 1,
): Promise<SearchResponse<LabelSearchItem>> {
  return searchLabels(q, repositoryId, 20, token, page);
}
