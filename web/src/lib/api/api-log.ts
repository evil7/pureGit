...
export function logMainRequest(
  ...
  size?: number,
  /**
   * 预捕获的降级状态（供 trackedFetch 异步日志用）：REST 日志在 res.clone().text()
   * 微任务里才输出，此时 withRestFallback 已 end() 弹栈，动态查 inFallback() 会误判为 false
   * 而丢失 ↪ 图标——故由调用方在请求发起时同步捕获传入。缺省则动态查 inFallback()。
   */
  fallbackOverride?: boolean,
): void {
  if (!isDev()) return;
  const sizeStr = size != null ? ` ${fmtSize(size)}` : "";
  const icon = (fallbackOverride !== undefined ? fallbackOverride : inFallback()) ? "↪ " : "";
