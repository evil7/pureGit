...
import type { Repository, OrgMember, OrgMemberWithRole, OrgTeam, OrgDetail } from "../restapi";
...
/**
 * 用户 Watching 的仓库（用户主页 Watching tab）：GraphQL 无适配（User 无 watchedRepositories
 * 连接）→ REST-only。列表短（通常 <100），一次拉全，不分页。
 */
export async function fetchUserWatchedSmart(
  login: string,
  token?: string | null,
): Promise<Repository[]> {
  return fetchUserWatched(login, 100, token, 1);
}
...
export { fetchOrgInvitations, createOrgInvitation, cancelOrgInvitation } from "../restapi";
export { setOrgMemberRole, removeOrgMember } from "../restapi";
export { createOrgTeam, updateOrgTeam, deleteOrgTeam } from "../restapi";
export { addTeamMember, removeTeamMember } from "../restapi";
