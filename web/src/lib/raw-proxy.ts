...
/** 直连 raw 的 5s 快速失败（被墙/超时立即切 /$raw 代理，不拖慢体验） */
...
/**
 * 智能获取 raw 文件文本（修订：allowProxy 参数——匿名保底仅直连）。
 * 通道：直连 raw.githubusercontent.com →（可选）/$raw 代理降级。
 * - directFirst=true（默认）：先直连（CORS 可用时最快路径）；失败转代理
 * - directFirst=false：跳过直连，直接代理（登录保底——受限网络直连超时浪费 5s）
 * - allowProxy=false：直连失败直接返回 null，不转代理（匿名保底「原始直连」，用户定稿 ）
...
export async function fetchRawContentSmart(
  ...
  // ① 直连 raw（CORS 可用时最快路径；directFirst=false 跳过——如受限网络）
  if (directFirst) {
    try {
      ...
      if (res.ok) {
        ...
        if (text.length <= RAW_MAX_BYTES) return text;
      }
    } catch {
      /* 直连失败 */
    }
  }
  // ② /$raw 代理（同源 worker，无 CORS；服务端绕墙）——allowProxy=false 时仅直连
