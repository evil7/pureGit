 ...
 *
 * 第一行 = TreeSearchInput（搜索框）；**第二行** = 本组件：数据源版本徽章
 * （纯数字版本号，hover 显示完整数据源描述如 `schema data by @octokit/openapi@22.0.0`）
 * + 刷新按钮。
 *
...
interface SchemaHeaderProps {
  /** 版本徽章文本（纯数字版本，如 "22.0.0"） */
  version: string;
  /** 版本 hover 描述（数据源说明，如 "schema data by @octokit/openapi@22.0.0"） */
  versionDesc?: string;
      ...
      <div className="flex items-center gap-1">
        ...
        {onRefresh && (
          <Button
            ...
            variant="ghost"
            size="icon"
            className="ml-auto h-5 w-5 rounded px-0 text-muted-foreground hover:bg-accent hover:text-foreground"
            onClick={onRefresh}
            disabled={loading}
            title={refreshTitle}
          >
