...
import { FeedCard } from "@/pages/home/Feed";
...
function ProfileView({ login, token }: { login: string; token: string | null }) {
  ...
  // Activity 公开活动流（仅用户页）：首次切到 activity tab 时拉第 1 页；后续点击加载更多续接
  const loadActivity = async (page: number) => {
    if (!kind || kind !== "user" || activityLoading || activityEnd) return;
    setActivityLoading(true);
    try {
      const batch = await fetchUserPublicEvents(login, token, 20, page);
      setActivity((prev) => (prev === null ? batch : [...prev, ...batch]));
      setActivityPage(page + 1);
      if (batch.length < 20) setActivityEnd(true);
    } catch {
      // 公开活动流失败静默（匿名限流/无数据 → 空态）；已有数据时不打断浏览
      if (activity === null) setActivity([]);
      setActivityEnd(true);
    } finally {
  ...
  useEffect(() => {
    if (tab === "activity" && kind === "user" && activity === null && !activityLoading) {
      void loadActivity(1);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, kind, activity, activityLoading]);
  ...
  return (
    ...
    <div className={PAGE_SHELL}>
      ...
      >
        ...
        {/* Activity（用户页）：公开活动流（list-public-events-for-user；复用首页 FeedCard） */}
        {tab === "activity" && (
          <>
            {activity === null ? (
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5 border-t pt-3 text-sm text-muted-foreground">
        ...
        {/* gists 入口（用户页；公开 gists，匿名可访问） */}
        {kind === "user" && (
          <span className="flex items-center gap-1.5">
            <FileCode2 className="size-3.5 shrink-0" />
            <Link to={`/gist?user=${login}`} className="text-primary hover:underline">
              {t("profile.gists")}
            </Link>
          </span>
        )}
