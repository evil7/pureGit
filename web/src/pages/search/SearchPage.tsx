...
type SearchTab = "repos" | "issues" | "prs" | "users" | "discussions" | "code" | "commits";

/** tab 顺序（对齐官方：Repos→Issues→PR→Users→Discussions→Code→Commits；code/commits GraphQL 无 SearchType 走 REST） */
const TABS: { value: SearchTab; labelKey: SearchLabelKey }[] = [
  ...
  { value: "code", labelKey: "search.tab.code" },
  { value: "commits", labelKey: "search.tab.commits" },
];
...
export default function SearchPage() {
  ...
  const [codes, setCodes] = useState<CodeSearchItem[]>([]);
  const [commits, setCommits] = useState<CommitSearchItem[]>([]);
    ...
    (async () => {
      switch (tab) {
        ...
        case "discussions":
          ...
          break;
        case "code":
          apply(await searchCodeSmart(q, token, page), setCodes);
          break;
        case "commits":
          apply(await searchCommitsSmart(q, token, page), setCommits);
          break;
      }
    })()
      .catch((e) => {
    ...
    <div className={PAGE_SHELL}>
      ...
      {!loading && !error && q.trim() && (
        <div>
          ...
          {tab === "code" &&
            renderResults(codes, (c) => (
              <Card key={`${c.repository.full_name}:${c.path}`}>
                <CardContent className="space-y-1 p-4">
                  <Link
                    to={`/${c.repository.full_name}/blob/HEAD/${c.path}`}
                    className="block truncate font-mono text-sm text-primary hover:underline"
                  >
                    {c.path}
                  </Link>
                  <p className="text-xs text-muted-foreground">{c.repository.full_name}</p>
                </CardContent>
              </Card>
            ))}
          {tab === "commits" &&
            renderResults(commits, (c) => (
              <Card key={`${c.repository.full_name}:${c.sha}`}>
                <CardContent className="space-y-1 p-4">
                  <Link
                    to={`/${c.repository.full_name}/commit/${c.sha}`}
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span className="truncate">{c.repository.full_name}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
