...
import { IssueSidebarActions } from "@/components/IssueSidebarActions";
      ...
      right={{
        node: (
          <aside className="space-y-5 text-sm">
            ...
            {/* Sub-issues（REST-only：子任务列表 + 父引用 + 添加/移除） */}
            <SubIssuesSection
              owner={owner!}
              repo={repo!}
              number={Number(number)}
              title={t("issueDetail.subIssues")}
              emptyText={t("issueDetail.noSubIssues")}
            />

            {/* Dependencies（REST-only：blocked by 依赖列表 + 添加/移除） */}
            <DependenciesSection
              owner={owner!}
              repo={repo!}
              number={Number(number)}
              title={t("issueDetail.dependencies")}
              emptyText={t("issueDetail.noDependencies")}
            />
            ...
            {token && (
              <div className="space-y-1 border-t pt-3">
                ...
                <IssueSidebarActions
                  ...
                  isPublicRepo={repoData ? !repoData.private : false}
                  onLockedChange={(locked) =>
                    setIssue((prev) => (prev ? { ...prev, locked } : prev))
                  }
                  onPinnedChange={(pinned) =>
                    setIssue((prev) => (prev ? { ...prev, isPinned: pinned } : prev))
                  }
                />
              </div>
            )}
                  ...
                  setIssue(
                    (prev) =>
                      prev && {
                        ...prev,
                        reactions,
                        viewerHasReacted: reactions.some((r) => r.viewerHasReacted),
                      },
                  )
                }
              />
