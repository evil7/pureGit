...
import { GitBranch, Search, Pencil } from "lucide-react";
...
export default function BranchesPage() {
  ...
  }, [owner, repo, defaultName, token, reloadKey]);
    ...
    <div>
      {defaultBranch && (
        <section className="mb-6">
          ...
          <BranchList
            ...
            defaultName={defaultBranch.name}
            fmt={fmt}
            onRenamed={onRenamed}
          />
        </section>
      ...
      <section>
        ...
        {shown.length === 0 ? (
          ...
          <>
            <BranchList branches={shown} defaultName="" fmt={fmt} onRenamed={onRenamed} />
...
/** 分支列表（通用行渲染：分支名 + 默认徽章 + ahead/behind + 更新时间 + 最后提交信息/作者 + 重命名） */
function BranchList({
  branches,
  defaultName,
  fmt,
  ...
  const confirmRename = async () => {
    if (!token || !renameTarget || renameBusy || !renameName.trim()) return;
    setRenameBusy(true);
    try {
      await renameBranchSmart(owner, repo, renameTarget.name, renameName.trim(), token);
      toastSuccess(t("branches.rename"));
      setRenameTarget(null);
      setRenameName("");
      onRenamed?.();
    } catch (e) {
      toastError(apiErrorMessage(e, t("branches.renameFailed")));
    } finally {
      setRenameBusy(false);
    }
  };
    ...
    <div className="overflow-hidden rounded-lg border">
      ...
      <Dialog open={renameTarget !== null} onOpenChange={(open) => !open && setRenameTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("branches.renameDialog.title")}</DialogTitle>
            <DialogDescription>{t("branches.renameDialog.desc")}</DialogDescription>
          </DialogHeader>
          <Input
            value={renameName}
            onChange={(e) => setRenameName(e.target.value)}
            placeholder={t("branches.renameDialog.placeholder")}
            autoFocus
          />
          <DialogFooter>
            <Button
            ...
            >
              {renameBusy ? t("common.saving") : t("branches.renameDialog.confirm")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
