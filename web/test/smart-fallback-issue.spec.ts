...
import { fetchRepoFilterDataSmart } from "@/lib/api/api-repo-filter";
import {
...
const mockLockIssue = vi.mocked(lockIssue);
const mockUnlockIssue = vi.mocked(unlockIssue);
...
describe("setIssueLockedSmart（GraphQL lockLockable 主通道 + REST 熔断）", () => {
  it("GraphQL 成功 → 前置查 id + lockLockable（REST 不调用）", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({
        data: { lockLockable: { lockedRecord: { id: "I_42", locked: true } } },
      } as never);
    await setIssueLockedSmart("evil7", "puregit", 42, true, "gho_x");
    expect(mockGraphql).toHaveBeenCalledTimes(2);
    expect(mockLockIssue).not.toHaveBeenCalled();
  });

  it("mutation errors → 熔断降级 REST lockIssue", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({ errors: [{ message: "boom" }] } as never);
    await setIssueLockedSmart("evil7", "puregit", 42, true, "gho_x");
    expect(mockLockIssue).toHaveBeenCalledWith("evil7", "puregit", 42, "gho_x");
  });

  it("解锁 → REST unlockIssue", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({ errors: [{ message: "boom" }] } as never);
    await setIssueLockedSmart("evil7", "puregit", 42, false, "gho_x");
    expect(mockUnlockIssue).toHaveBeenCalledWith("evil7", "puregit", 42, "gho_x");
  });
});

describe("deleteIssueSmart（GraphQL-only 无 REST 降级）", () => {
  it("GraphQL 成功 → 前置查 id + deleteIssue", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({
        data: { deleteIssue: { repository: { nameWithOwner: "evil7/puregit" } } },
      } as never);
    await deleteIssueSmart("evil7", "puregit", 42, "gho_x");
    expect(mockGraphql).toHaveBeenCalledTimes(2);
  });

  it("mutation errors → 抛错（无 REST）", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({ errors: [{ message: "boom" }] } as never);
    await expect(deleteIssueSmart("evil7", "puregit", 42, "gho_x")).rejects.toThrow();
  });

  it("前置查 id 失败 → 抛错", async () => {
    mockGraphql.mockResolvedValue({ data: { repository: { issue: null } } } as never);
    await expect(deleteIssueSmart("evil7", "puregit", 42, "gho_x")).rejects.toThrow();
  });
});

describe("setIssuePinnedSmart / transferIssueSmart（GraphQL-only 无 REST 降级）", () => {
  it("pin：前置查 id 失败 → 抛错（无 REST）", async () => {
    mockGraphql.mockResolvedValue({ data: { repository: { issue: null } } } as never);
    await expect(setIssuePinnedSmart("evil7", "puregit", 42, true, "gho_x")).rejects.toThrow();
  });

  it("transfer：成功返回目标 issue url", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({ data: { repository: { id: "R_7" } } } as never)
      .mockResolvedValueOnce({
        data: {
          transferIssue: {
            issue: { id: "I_42", url: "https://github.com/evil7/target/issues/42" },
          },
        },
      } as never);
    const url = await transferIssueSmart("evil7", "puregit", 42, "evil7", "target", "gho_x");
    expect(url).toContain("issues/42");
  });

  it("transfer：目标仓库不存在 → 抛错", async () => {
    mockGraphql
      .mockResolvedValueOnce({ data: { repository: { issue: { id: "I_42" } } } } as never)
      .mockResolvedValueOnce({ data: { repository: null } } as never);
    await expect(
      transferIssueSmart("evil7", "puregit", 42, "evil7", "nope", "gho_x"),
    ).rejects.toThrow();
  });
});
